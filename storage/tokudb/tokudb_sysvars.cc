...
static MYSQL_SYSVAR_BOOL(debug_pause_background_job_manager,
    ...
    0, "TokuDB debug : pause the background job manager",
    NULL, NULL, FALSE);

#endif // TOKUDB_DEBUG

static MYSQL_SYSVAR_ULONGLONG(cache_size, cache_size,
...
static MYSQL_SYSVAR_INT(cardinality_scale_percent, cardinality_scale_percent,
    0, "TokuDB index cardinality scale percentage",
    NULL, NULL, 50,
    0, 100, 0);

static MYSQL_SYSVAR_BOOL(checkpoint_on_flush_logs, checkpoint_on_flush_logs,
...
static void checkpointing_period_update(THD* thd, st_mysql_sys_var* sys_var,
...
static MYSQL_SYSVAR_UINT(checkpointing_period, checkpointing_period,
    0, "TokuDB Checkpointing period",
    NULL, checkpointing_period_update, 60,
    0, ~0U, 0);

static void cleaner_iterations_update(THD* thd, st_mysql_sys_var* sys_var,
                                      void* var, const void* save) {
    ulong* ci = (ulong*)var;
    *ci = *(const ulong*)save;
...
static MYSQL_SYSVAR_ULONG(cleaner_iterations, cleaner_iterations,
    0, "TokuDB cleaner_iterations",
    NULL, cleaner_iterations_update, DEFAULT_TOKUDB_CLEANER_ITERATIONS,
    0, ~0UL, 0);

static void cleaner_period_update(THD* thd, st_mysql_sys_var* sys_var,
                                  void* var, const void * save) {
    ulong* cp = (ulong*)var;
    *cp = *(const ulong*)save;
...
static MYSQL_SYSVAR_ULONG(cleaner_period, cleaner_period,
    0, "TokuDB cleaner_period",
    NULL, cleaner_period_update, DEFAULT_TOKUDB_CLEANER_PERIOD,
    0, ~0UL, 0);
...
static MYSQL_SYSVAR_STR(data_dir, data_dir,
    PLUGIN_VAR_READONLY, "TokuDB Data Directory",
    NULL, NULL, NULL);

static MYSQL_SYSVAR_ULONG(debug, debug,
    0, "TokuDB Debug",
    NULL, NULL, 0,
    0, ~0UL, 0);

static MYSQL_SYSVAR_BOOL(directio, directio,
    PLUGIN_VAR_READONLY, "TokuDB Enable Direct I/O ",
    NULL, NULL, FALSE);

static void enable_partial_eviction_update(THD* thd, st_mysql_sys_var* sys_var,
                                           void* var, const void* save) {
...
static MYSQL_SYSVAR_BOOL(enable_partial_eviction, enable_partial_eviction,
    0, "TokuDB enable partial node eviction",
    NULL, enable_partial_eviction_update, TRUE);
...
static void fsync_log_period_update(THD* thd, st_mysql_sys_var* sys_var,
                                    void* var, const void* save) {
    uint* flp = (uint*)var;
    *flp = *(const uint*)save;
...
static MYSQL_SYSVAR_UINT(fsync_log_period, fsync_log_period,
    0, "TokuDB fsync log period",
    NULL, fsync_log_period_update, 0,
    0, ~0U, 0);

static MYSQL_SYSVAR_STR(log_dir, log_dir,
...
static MYSQL_SYSVAR_UINT(read_status_frequency, read_status_frequency,
    0, "TokuDB frequency that show processlist updates status of reads",
    NULL, NULL, 10000,
    0, ~0U, 0);

static MYSQL_SYSVAR_STR(tmp_dir, tmp_dir,
...
static MYSQL_SYSVAR_UINT(write_status_frequency, write_status_frequency,
    0, "TokuDB frequency that show processlist updates status of writes",
    NULL, NULL, 1000,
    0, ~0U, 0);

#if TOKU_INCLUDE_HANDLERTON_HANDLE_FATAL_SIGNAL
static MYSQL_SYSVAR_STR(gdb_path, gdb_path,
    PLUGIN_VAR_READONLY|PLUGIN_VAR_RQCMDARG, "TokuDB path to gdb for extra debug info on fatal signal",
    NULL, NULL, "/usr/bin/gdb");

static MYSQL_SYSVAR_BOOL(gdb_on_fatal, gdb_on_fatal,
    0, "TokuDB enable gdb debug info on fatal signal",
    NULL, NULL, true);
#endif

#if TOKUDB_CHECK_JEMALLOC
static MYSQL_SYSVAR_UINT(check_jemalloc, check_jemalloc,
    0, "Check if jemalloc is linked",
    NULL, NULL, 1, 0, 1, 0);
#endif
...
static MYSQL_THDVAR_BOOL(analyze_in_background,
    0, "Dispatch ANALYZE TABLE to background job.",
    NULL, NULL, false);
...
static MYSQL_THDVAR_ENUM(analyze_mode,
    ...
    "TOKUDB_ANALYZE_RECOUNT_ROWS perform logical recount of the rows in "
    "TOKUDB_ANALYZE_CANCEL terminate and cancel all scheduled background jobs "
    "for a table",
    NULL, NULL, TOKUDB_ANALYZE_STANDARD,
    &tokudb_analyze_mode_typelib);

static MYSQL_THDVAR_ULONGLONG(analyze_throttle,
    0, "analyze throttle (keys)",
    NULL, NULL, 0, 0, ~0U, 1);

static MYSQL_THDVAR_ULONGLONG(analyze_time,
    0, "analyze time (seconds)",
    NULL, NULL, 5, 0, ~0U, 1);

static MYSQL_THDVAR_ULONGLONG(auto_analyze,
    0, "auto analyze threshold (percent)",
    NULL, NULL, 0, 0, ~0U, 1);
...
static void checkpoint_lock_update(THD* thd, st_mysql_sys_var* var,
...
static MYSQL_THDVAR_BOOL(checkpoint_lock,
    ...
    NULL, checkpoint_lock_update, false);
