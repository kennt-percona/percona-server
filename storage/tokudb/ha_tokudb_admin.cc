...
public:
    ...
    void initialize(THD* thd, TOKUDB_SHARE* share, DB_TXN* txn);
    ...
    void status(char* database, char* table, char* type, char* params,
                char* status);
...
void recount_rows_t::initialize(THD* thd, TOKUDB_SHARE* share, DB_TXN* txn) {
    ...
    _result = HA_ADMIN_OK;
    _recount_start = _total_elapsed_time = 0;

    _rows = _deleted_rows = _ticks = 0;
}
void recount_rows_t::run(void) {
...
void recount_rows_t::cancel(void) {
    _cancel = true;
}
void recount_rows_t::destroy(void) {
    _share->release();
}
void recount_rows_t::free(void) {
    delete this;
}
const char* recount_rows_t::key(void) {
    return _share->full_table_name();
}
void recount_rows_t::status(char* database, char* table, char* type,
                            char* params, char* status) {
...
int recount_rows_t::analyze_recount_rows_progress(uint64_t count, uint64_t deleted, void* extra) {
    recount_rows_t* context = (recount_rows_t*)extra;
    return context->analyze_recount_rows_progress(count, deleted);
}
int recount_rows_t::analyze_recount_rows_progress(uint64_t count, uint64_t deleted) {
    _rows = count;
    ...
    if (_ticks > 1000) {
        ...
        if ((_thd && thd_killed(_thd)) || _cancel) {
            // client killed
        ...
        // throttle
        // given the throttle value, lets calculate the maximum number of rows we
        // should have seen so far in a .1 sec resolution
        if (_throttle > 0) {
            ...
            if (_rows + _deleted_rows > estimated_rows) {
                tokudb::time::sleep_microsec(100000); // sleep for 1/10 of a second
            }
        }
    }
    return 0;
...
void recount_rows_t::get_analyze_status(char* msg) {
    sprintf(msg, "recount_rows %s.%s counted %llu rows and %llu deleted in "
            "%llu seconds.", _share->database_name(), _share->table_name(),
            _rows, _deleted_rows, _total_elapsed_time /
            tokudb::time::MICROSECONDS);
}
...
void standard_t::initialize(THD* thd, TOKUDB_SHARE* share, DB_TXN* txn) {
    assert_debug(thd != NULL);
    assert_debug(share != NULL);
    ...
    _share = share;
    assert_always(_share->state() == TOKUDB_SHARE::OPENED);
    _throttle = tokudb::sysvars::analyze_throttle(thd);
    ...
    _analyze_start = _total_elapsed_time = 0;
    _rows = _deleted_rows = _ticks = _analyze_key_start = _key_elapsed_time = 0;
    _result = HA_ADMIN_OK;
}
void standard_t::run(void) {
    ...
    for (ulonglong current_key = 0;
        ...
        if (_thd && (_result == HA_ADMIN_FAILED ||
            (double)_deleted_rows > _delete_fraction * (_rows + _deleted_rows))) {
            ...
            rowmsglen = snprintf(rowmsg, sizeof rowmsg, "rows processed %llu "
                                 "rows deleted %llu", _rows, _deleted_rows);
...
void standard_t::cancel(void) {
    _cancel = true;
}
void standard_t::destroy(void) {
    _share->lock();
    _share->allow_auto_analysis(false);
    _share->unlock();
    _share->release();
}
void standard_t::free(void) {
    delete this;
}
const char* standard_t::key(void) {
    return _share->full_table_name();
}
void standard_t::status(char* database, char* table, char* type, char* params,
                        char* status) {
    strcpy(database, _share->database_name());
    strcpy(table, _share->table_name());
    strcpy(type, "TOKUDB_ANALYZE_MODE_STANDARD");
    sprintf(params, "TOKUDB_ANALYZE_DELETE_FRACTION=%f; "
...
void standard_t::get_analyze_status(char* msg) {
    ...
    sprintf(msg, "analyze table standard %s.%s.%s %llu of %u %.lf%% rows %.lf%% time, %s",
        _share->database_name(), _share->table_name(),
        _share->_key_descriptors[_current_key]._name,
        _current_key, _share->_keys, progress_rows * 100.0,
        progress_time * 100.0, scan_direction);
}
int standard_t::analyze_key_progress(void) {
    if (_ticks > 1000) {
        ...
        if ((_thd && thd_killed(_thd)) || _cancel) {
            // client killed
            return ER_ABORTING_CONNECTION;
        } else if(_time_limit > 0 && (uint64_t)_key_elapsed_time > _time_limit) {
            // time limit reached
            return ETIME;
        }
...
int standard_t::analyze_key(uint64_t* rec_per_key_part) {
    ...
    while (1) {
        if (cursor == NULL) {
            ...
            cursor->c_set_check_interrupt_callback(cursor,
        ...
        if (copy_key) {
            prev_key.data = tokudb::memory::realloc(prev_key.data, key.size,
        ...
        if (TOKUDB_UNLIKELY(_half_time > 0 && _scan_direction == DB_NEXT &&
            _key_elapsed_time >= _half_time && _rows < _half_rows)) {
...
int TOKUDB_SHARE::analyze_recount_rows(THD* thd,DB_TXN* txn) {
    ...
    tokudb::analyze::recount_rows_t* job = new tokudb::analyze::recount_rows_t;
    assert_always(job != NULL);
    job->initialize(thd, this, txn);

    // job->destroy will drop the ref
    addref();
    unlock();

    uint64_t id = tokudb::background::_job_manager->
        run_job(job, true, tokudb::sysvars::analyze_in_background(thd));
...
int TOKUDB_SHARE::analyze_standard(THD* thd, DB_TXN* txn) {
    ...
    uint64_t id = tokudb::background::_job_manager->
        run_job(job, txn == NULL ? false : true,
...
int ha_tokudb::check(THD *thd, HA_CHECK_OPT *check_opt) {
    ...
    if (result == HA_ADMIN_OK) {
        ...
        snprintf(write_status_msg, sizeof write_status_msg,
                 "%s primary=%d num=%d", share->table_name(), primary_key,
                 num_DBs);
        if (TOKUDB_UNLIKELY(tokudb::sysvars::debug & TOKUDB_DEBUG_CHECK)) {
        ...
        for (uint i = 0; i < num_DBs; i++) {
            DB *db = share->key_file[i];
            const char *kname = i == primary_key ? "primary" :
            ...
            r = db->verify_with_progress(db, ha_tokudb_check_progress,
                                         &check_context,
                                         (tokudb::sysvars::debug &
                                          TOKUDB_DEBUG_CHECK) != 0, keep_going);
            if (r != 0) {
            ...
            snprintf(write_status_msg, sizeof write_status_msg,
                     "%s key=%s %u result=%d", share->table_name(), kname,
                     i, r);
            thd_proc_info(thd, write_status_msg);
            if (TOKUDB_UNLIKELY(tokudb::sysvars::debug & TOKUDB_DEBUG_CHECK)) {
                ha_tokudb_check_info(thd, table, write_status_msg);
