...
int trx_callback(DB_TXN* txn, iterate_row_locks_callback iterate_locks,
...
int lock_waits_callback(DB* db, uint64_t requesting_txnid, const DBT* left_key,
                        const DBT* right_key, uint64_t blocking_txnid,
                        uint64_t start_time, void *extra) {

    lock_waits_extra_t* e = reinterpret_cast<struct lock_waits_extra_t*>(extra);
    THD* thd = e->thd;
    TABLE* table = e->table;
...
int lock_waits_fill_table(THD* thd, TABLE_LIST* tables, COND* cond) {
    ...
    if (!tokudb_hton_initialized) {
        ...
        error = db_env->iterate_pending_lock_requests(db_env, lock_waits_callback, &e);
        if (error)
            my_error(ER_GET_ERRNO, MYF(0), error, tokudb_hton_name);
    }
...
int locks_callback(DB_TXN* txn, iterate_row_locks_callback iterate_locks,
                   void* locks_extra, void* extra) {
    uint64_t txn_id = txn->id64(txn);
    uint64_t client_id = txn->get_client_id(txn);
...
int report_fractal_tree_info(TABLE* table, THD* thd) {
    ...
    while (error == 0) {
        error = tmp_cursor->c_get(tmp_cursor, &curr_key, &curr_val, DB_NEXT);
        if (!error) {
            error = report_fractal_tree_info_for_db(&curr_key, &curr_val, table, thd);
            if (error)
...
int report_fractal_tree_block_map_iterator(uint64_t checkpoint_count,
                                           int64_t num_rows, int64_t blocknum,
                                           int64_t diskoff, int64_t size,
                                           void* iter_extra) {
    ...
    assert(num_rows > 0);
    if (e->num_rows == 0) {
        e->checkpoint_counts = (uint64_t*)tokudb::memory::malloc(num_rows * (sizeof *e->checkpoint_counts),
                                                             MYF(MY_WME|MY_ZEROFILL|MY_FAE));
        e->blocknums = (int64_t*)tokudb::memory::malloc(num_rows * (sizeof *e->blocknums),
                                                    MYF(MY_WME|MY_ZEROFILL|MY_FAE));
        e->diskoffs = (int64_t*)tokudb::memory::malloc(num_rows * (sizeof *e->diskoffs),
                                                   MYF(MY_WME|MY_ZEROFILL|MY_FAE));
        e->sizes = (int64_t*)tokudb::memory::malloc(num_rows * (sizeof *e->sizes),
                                                MYF(MY_WME|MY_ZEROFILL|MY_FAE));
        e->num_rows = num_rows;
    }
...
int report_fractal_tree_block_map_for_db(const DBT* dname, const DBT* iname,
    ...
    error = db->iterate_fractal_tree_block_map(db, report_fractal_tree_block_map_iterator, &e);
    {
        int close_error = db->close(db, 0);
        if (!error) {
            error = close_error;
...
int report_fractal_tree_block_map(TABLE* table, THD* thd) {
    ...
    while (error == 0) {
        error = tmp_cursor->c_get(tmp_cursor, &curr_key, &curr_val, DB_NEXT);
        if (!error) {
            error = report_fractal_tree_block_map_for_db(&curr_key, &curr_val, table, thd);
        }
        if (!error && thd_killed(thd))
            error = ER_QUERY_INTERRUPTED;
    }
...
#if MYSQL_VERSION_ID >= 50600
int fractal_tree_block_map_fill_table(THD* thd, TABLE_LIST* tables, Item* cond) {
#else
int fractal_tree_block_map_fill_table(THD* thd, TABLE_LIST* tables, COND* cond) {
#endif
    TOKUDB_DBUG_ENTER("");
    int error;
    TABLE* table = tables->table;
...
void background_job_status_callback(uint64_t id, const char* database_name,
                                    const char* table_name, const char* type,
                                    const char* params, const char* status,
                                    bool user_scheduled, time_t scheduled_time,
                                    time_t started_time, void* extra) {
    ...
    table->field[1]->store(database_name, strlen(database_name),
                           system_charset_info);
...
int report_background_job_status(TABLE *table, THD *thd) {
    ...
    tokudb::background::_job_manager->
        iterate_jobs(background_job_status_callback, &extra);
    return error;
}
